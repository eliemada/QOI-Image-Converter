...
public final class QOIDecoder {
    ...
    public static int[] decodeHeader(byte[] header){
        ...
        assert (header.length == QOISpecification.HEADER_SIZE) :
                "The length of the header is not equal to the constant HEADER_SIZE" ;
        assert ArrayUtils.equals(
                ArrayUtils.extract(header, 0, 4), QOISpecification.QOI_MAGIC)
                : "The first four bytes are not equal to the constant HEADER_SIZE" ;
        assert (header[header.length-2]==QOISpecification.RGB||
                header[header.length-2]==QOISpecification.RGBA) :
                "The number of channels is not equal to RGB or RGBA";
        assert (header[header.length-1]==QOISpecification.ALL||
                header[header.length-1]==QOISpecification.sRGB) :
                "The color space is not equal to ALL or sRGB";
    ...
    public static int decodeQoiOpRGB(byte[][] buffer, byte[] input, byte alpha, int position, int idx){
        assert input != null && buffer !=null : "The input and/or the buffer are null";
    ...
    public static int decodeQoiOpRun(byte[][] buffer, byte[] pixel, byte chunk, int position){
        assert buffer != null && pixel != null : "The buffer and/or the pixel are null";
        assert (position >= 0 && position < buffer.length) :
